<?php
    ...
    if (isset($_REQUEST['btn_update'])) {
       try {
            ...
            $directory = "upload/"; //set upload folder path for update time previos file remove and new file upload for next use

            if ($image_file) {
                if ($type == "image/jpg" || $type == 'image/jpeg' || $type == "image/png" || $type == "imagegif") {
                    if (!file_exists($path)) { //check file not exist in your upload folder path
                        if ($size < 5000000) { //check file size 5MB
                            unlink($directory.$row['image']); //unlink function remove previos file
...
<div class="card border-light mb-3">
  <div class="card-header display-4">Edit File Form</div>
  <div class="card-body">
  <?php
    if(isset($errorMsg)) {
?>
    ...
    <div class="form-group text-center">
        ...
        <label for="file" class="control-label col-sm-3">File</label>
            <div class="col-sm-8">
                ...
                <p>
                    <img src="upload/<?php echo $image;?>" width="100px" alt="">
                </p>
            </div>
        </div>
    </div>
